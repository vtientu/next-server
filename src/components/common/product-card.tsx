    ...
    <div className='flex flex-col px-4 mb-10'>
      ...
      >
        <Image
          ...
          alt={shoes.name}
          src={shoes.thumbnail}
          className={cn('object-contain', shoes?.thumbnail_hover && 'hover:opacity-0')}
          priority
        />
      ...
      <div className='flex flex-col gap-5 text-center'>
        <Link href={`/product/${shoes.id}`} className='font-semibold cursor-pointer leading-[22px] h-[44px]'>
          {shoes.name}
        </Link>
        <div className='flex items-center justify-center space-x-2 flex-wrap text-[15px]'>
          <p className='text-md font-semibold text-[#f00]'>{formatPrice(shoes.price_discount)}</p>
          <p className='text-md line-through text-gray-400'>{formatPrice(shoes.price)}</p>
          <p className='text-sm font-semibold text-[#f00]'>[ -{shoes.percent_discount}%]</p>
        </div>
      </div>
    </div>
  )
